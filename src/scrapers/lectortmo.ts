...
export class lectortmoClass extends Scraper {
	constructor() {
		super();
		this.provider = "Lectortmo";
		this.canSearch = true;
		this.nsfw = false;
	}
	...
	public async search(query: string, options?: Partial<SearchOptions>) {
		...
		if (query === "") {
			// Get popular page
			pageUrl = "https://lectortmo.com/";
		} else {
			pageUrl = `https://lectortmo.com//library?${encodeURIComponent(query)}`;
		}
		...
		const searchResultData: ScraperResponse[] = await Promise.all(
			ids.map((id) => updateManga(this.provider, id))
		);
...
// Generate lectortmo object and export it
const lectortmo = new lectortmoClass();
export default lectortmo;
